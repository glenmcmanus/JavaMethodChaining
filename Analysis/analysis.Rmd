...
method_size_count <- c(classified$Accessor, classified$Builder, classified$Assertion, classified$Others)

classified_bins <- data.frame(size_id,method_type,method_size_count)
classified_bins
classified

ggplot(classified_bins, aes(fill=method_type, y=method_size_count, x=size_id)) + geom_bar(position="dodge", stat="identity") + scale_y_log10()
